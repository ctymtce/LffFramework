...
class CFs {
    ...
    static function writeLogs($content, $path='/tmp/l.log', $mod='a')
    {
        $content = date("Y-m-d H:i:s ") . $content . "\n";
        return $this->writeFile($path, $content, $mod);
    }
    static function writeFile($path, $content=null, $mod='w')
    {
        $fp = fopen($path, $mod);
        if(!$fp) return false;
    ...
    static function getDirInfo($dir, $ignoreArr=array()) 
    {
    ...
    static function walkdir($dir, &$subDirs=array(), $depth=1) 
    {
        if(!is_dir($dir)) return;
        $subDirs[] = $dir;
        $handler = opendir($dir);
    ...
    static function makeDirs($dir, $isrevers=false)
    {
    ...
    *
    */
    static function getFPerms($path)
    {
        $permArr = array();
    ...
    static function head($path, $len=128)
    {
        if(!is_file($path))return null;
        $fp = fopen($path, "r");
        if(!$fp) return false;
    ...
    static function wcl($filename)
    {
        $fp = fopen($filename, "r");
        // $size = 1024*1024; //1048576=1M
        $line = 0;
    ...
    static function mvFile($src, $dest, $overwrite='T')
    {
