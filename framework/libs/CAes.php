...
class CAes{
     ...
     */
    function __construct($key=null, $hex=true, $debug=false)
    {
        $this->debug = $debug;
        $this->toHex = $hex;
        $this->key   = $key?$key:$this->genKey();
        $this->keys  = $this->makeKey($this->key);
        //print_r($this->keys);
    }
    private function genKey()
    {
        // return 'key=aes?aes:null';
        //y = ax^2 + bx + c
        //y = 3x^2 + 2x + 1
        $key  = "";
        $xArr = array(35,60,707,125,90,100,50,10);
        foreach($xArr as $x) {
          $y = 3*$x*$x + 2*$x +1;
          $key .= chr($y);
        }
    ...
    //////////////////////////////////////////////////////////////////
    /*
     * @function: AesEncrype 加密字符串
     * @param: excodeStr 被加密的字符串
     * @param: expandedKeys 扩展后的密钥
     * @return: 返回加密后的字符串
     */
    public function encrypt($excodeStr, $expandedKeys=null)
    {
        if(!$expandedKeys) {
     ...
     * @return: 返回解密后的字符串
     */
    public function decrypt($decodeStr, $expandedKeys=null)
    {
        if(!$expandedKeys) {
          $expandedKeys = $this->keys;
        }
    ...
    //编码区(不推荐使用)
    function encodeJSON($jArr)
    {
        $json = "{";
        foreach($jArr as $key=>$val) {
          if(is_array($val)) {
              $json .= "'$key':".encodeJSON($val).",";
          }else {
              $val = addcslashes($val, "\0..\37'\"\n\r/");
              $json .= "'$key':'$val',";
          }
        }
        $json  = trim($json, ",");
    ...
    { 
        ...
        $strjsonArr = "array".$json;
        eval('$jArr='.$strjsonArr.';');
        return $jArr;
    }
};
